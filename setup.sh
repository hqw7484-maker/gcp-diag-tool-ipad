...
clear
echo -e "${CYAN}${BOLD}>>> [1/3] 启动物理环境净化序列...${RESET}"
tasks=("中断冗余进程" "强制解绑端口" "粉碎残留日志" "重置虚拟句柄")
for i in "${!tasks[@]}"; do
    case $i in
...
echo -e "${GREEN}✅ 净化完成。${RESET}\n"

echo -e "${CYAN}${BOLD}>>> [2/3] 注入 Xray-Core 极客驱动...${RESET}"
...
echo -e "${CYAN}${BOLD}>>> [3/3] 建立分流隧道连接...${RESET}"
# 网页监听在 8085，等待 Xray 的降级转发
...
for i in {1..15}; do
    ...
    if [ -n "$LINK" ]; then
        ...
        echo -e "🚀 ${GREEN}${BOLD}SYSTEM ONLINE | 混合模式就绪${RESET}"
        echo -e "${CYAN}--------------------------------------------${RESET}"
    ...
    echo -ne "   ${YELLOW}⌛ 打捞域名中... ($((i*2))s)${RESET}\r"
    sleep 2
done
